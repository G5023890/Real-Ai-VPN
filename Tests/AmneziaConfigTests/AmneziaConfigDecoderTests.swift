...
final class AmneziaConfigDecoderTests: XCTestCase {
    ...
    func testBuildSingBoxConfigFromShadowrocketVLESSReality() throws {
        ...
        let quicFallbackRuleIndex = try XCTUnwrap(rules.firstIndex { ($0["action"] as? String) == "reject" && ($0["network"] as? String) == "udp" })
        let quicFallbackRule = rules[quicFallbackRuleIndex]
        let yandexMapsQUICRuleIndex = try XCTUnwrap(rules.firstIndex { ($0["outbound"] as? String) == "proxy" && ($0["network"] as? String) == "udp" && ($0["port"] as? Int) == 443 })
        let yandexMapsQUICRule = rules[yandexMapsQUICRuleIndex]
        let yandexMapsQUICSuffixes = try XCTUnwrap(yandexMapsQUICRule["domain_suffix"] as? [String])
        let legacyMailRuleIndex = try XCTUnwrap(rules.firstIndex { ($0["outbound"] as? String) == "direct" && ($0["network"] as? String) == "tcp" && $0["port"] != nil })
        let legacyMailRule = rules[legacyMailRuleIndex]
        let proxyDomainRuleIndex = try XCTUnwrap(rules.firstIndex { ($0["outbound"] as? String) == "proxy" && $0["domain_suffix"] != nil && $0["network"] == nil })
        let proxyDomainRule = rules[proxyDomainRuleIndex]
        let proxyDomainSuffixes = try XCTUnwrap(proxyDomainRule["domain_suffix"] as? [String])
        ...
        let yandexMapsDNSRule = try XCTUnwrap(dnsRules.first { ($0["server"] as? String) == "cloudflare" && ($0["strategy"] as? String) == "ipv4_only" })
        let yandexMapsDNSSuffixes = try XCTUnwrap(yandexMapsDNSRule["domain_suffix"] as? [String])
        let forceRule = try XCTUnwrap(dnsRules.first { ($0["server"] as? String) == "cloudflare" && $0["strategy"] == nil })
        ...
        XCTAssertEqual(quicFallbackRule["port"] as? Int, 443)
        XCTAssertEqual(quicFallbackRule["method"] as? String, "default")
        XCTAssertEqual(quicFallbackRule["no_drop"] as? Bool, true)
        XCTAssertLessThan(yandexMapsQUICRuleIndex, quicFallbackRuleIndex)
        XCTAssertTrue(yandexMapsQUICSuffixes.contains("maps.yandex.ru"))
        XCTAssertTrue(yandexMapsQUICSuffixes.contains("maps.yandex.net"))
        XCTAssertTrue(yandexMapsQUICSuffixes.contains("yastatic.net"))
        XCTAssertTrue(yandexMapsQUICSuffixes.contains("mobile.maps.yandex.net"))
        XCTAssertTrue(yandexMapsQUICSuffixes.contains("core-renderer-tiles.maps.yandex.net"))
        XCTAssertLessThan(legacyMailRuleIndex, proxyDomainRuleIndex)
        XCTAssertEqual(legacyMailRule["port"] as? [Int], [25, 110, 143, 465, 585, 587, 993, 995])
        XCTAssertTrue(proxyDomainSuffixes.contains("google.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("gmail.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("googleapis.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("gstatic.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("apple.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("icloud.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("mzstatic.com"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yastatic.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yandex.st"))
        XCTAssertTrue(proxyDomainSuffixes.contains("maps.yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("maps.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("api-maps.yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("static-maps.yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("geocode-maps.yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("suggest-maps.yandex.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("mobile.maps.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("mobile.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("core-renderer-tiles.maps.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("vec.maps.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("sat.maps.yandex.net"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yandexnavi.ru"))
        XCTAssertTrue(proxyDomainSuffixes.contains("yandexcloud.net"))
        XCTAssertLessThan(quicFallbackRuleIndex, proxyDomainRuleIndex)
        XCTAssertTrue(directDomainSuffixes.contains("ru"))
        XCTAssertFalse(directDomainSuffixes.contains("gmail.com"))
        XCTAssertFalse(directDomainSuffixes.contains("google.com"))
        XCTAssertFalse(directDomainSuffixes.contains("apple.com"))
        XCTAssertFalse(directDomainSuffixes.contains("icloud.com"))
        XCTAssertFalse(directDomainSuffixes.contains("yandex.ru"))
        XCTAssertFalse(directDomainSuffixes.contains("maps.yandex.ru"))
        ...
        XCTAssertTrue(forceSuffixes.contains("google.com"))
        XCTAssertTrue(forceSuffixes.contains("gmail.com"))
        XCTAssertTrue(forceSuffixes.contains("googleapis.com"))
        XCTAssertTrue(forceSuffixes.contains("apple.com"))
        XCTAssertTrue(forceSuffixes.contains("icloud.com"))
        XCTAssertTrue(forceSuffixes.contains("mzstatic.com"))
        XCTAssertFalse(forceSuffixes.contains("yandex.ru"))
        XCTAssertFalse(forceSuffixes.contains("maps.yandex.ru"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("yandex.ru"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("maps.yandex.ru"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("api-maps.yandex.ru"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("static-maps.yandex.ru"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("yastatic.net"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("mobile.maps.yandex.net"))
        XCTAssertTrue(yandexMapsDNSSuffixes.contains("core-renderer-tiles.maps.yandex.net"))
        XCTAssertTrue(providerSuffixes.contains("ru"))
        XCTAssertFalse(providerSuffixes.contains("google.com"))
        XCTAssertFalse(providerSuffixes.contains("gmail.com"))
        XCTAssertFalse(providerSuffixes.contains("googleapis.com"))
        XCTAssertFalse(providerSuffixes.contains("apple.com"))
        XCTAssertFalse(providerSuffixes.contains("icloud.com"))
        XCTAssertFalse(providerSuffixes.contains("yandex.ru"))
        XCTAssertFalse(providerSuffixes.contains("maps.yandex.ru"))
    ...
    func testSingBoxDNSProtectionRoutesProviderDomainsToYandex() throws {
        ...
        let yandexMapsDNSRule = try XCTUnwrap(rules.first { ($0["server"] as? String) == "cloudflare" && ($0["strategy"] as? String) == "ipv4_only" })
        let yandexMapsDNSSuffixes = try XCTUnwrap(yandexMapsDNSRule["domain_suffix"] as? [String])
        let forceRule = try XCTUnwrap(rules.first { ($0["server"] as? String) == "cloudflare" && $0["strategy"] == nil })
        ...
        XCTAssertFalse(directIPCIDRs.contains("17.0.0.0/8"))
        ...
        XCTAssertTrue(forceSuffixes.contains("2ip.ru"))
        XCTAssertTrue(forceSuffixes.contains("google.com"))
        XCTAssertTrue(forceSuffixes.contains("gmail.com"))
        XCTAssertTrue(forceSuffixes.contains("apple.com"))
        XCTAssertTrue(forceSuffixes.contains("icloud.com"))
    ...
    func testSingBoxIPv4OnlyCompatibilityDisablesIPv6TunnelAddressAndDNSAAAA() throws {
        let config = try SingBoxConfigBuilder().build(
            from: makeVLESSProfile(),
            ipv4OnlyCompatibilityEnabled: true
        )
        let data = try XCTUnwrap(config.jsonString.data(using: .utf8))
        let root = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        let inbounds = try XCTUnwrap(root["inbounds"] as? [[String: Any]])
        let tunInbound = try XCTUnwrap(inbounds.first { ($0["type"] as? String) == "tun" })
        let tunAddresses = try XCTUnwrap(tunInbound["address"] as? [String])
        let dns = try XCTUnwrap(root["dns"] as? [String: Any])
        let dnsRules = try XCTUnwrap(dns["rules"] as? [[String: Any]])

        XCTAssertEqual(tunAddresses, ["172.19.0.1/30"])
        XCTAssertEqual(dns["strategy"] as? String, "ipv4_only")
        XCTAssertTrue(dnsRules.allSatisfy { ($0["strategy"] as? String) == "ipv4_only" })
    }
