...
final class AmneziaConfigDecoderTests: XCTestCase {
    ...
    func testBuildSingBoxConfigFromShadowrocketVLESSReality() throws {
        ...
        let quicFallbackRule = try XCTUnwrap(rules.first { ($0["action"] as? String) == "reject" && ($0["network"] as? String) == "udp" })
        ...
        let proxyDomainRuleIndex = try XCTUnwrap(rules.firstIndex { ($0["outbound"] as? String) == "proxy" && $0["domain_suffix"] != nil })
        ...
        let forceRule = try XCTUnwrap(dnsRules.first { ($0["server"] as? String) == "cloudflare" })
